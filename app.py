...
if st.session_state.page == "setup":
    ...
    with col2:
        ...
        if perc >= 85:
            feedback = f"Exceptional demonstration of mathematical mastery by {st.session_state.student_name}. The candidate displays high fluency in complex problem-solving and logical deduction. The results suggest a readiness for advanced competitive training or accelerated curriculum."
        elif perc >= 60:
            feedback = f"Commendable performance. {st.session_state.student_name} shows a strong conceptual foundation with minor inconsistencies in multi-step execution. Focused practice on high-complexity logic and speed will bridge the gap to full mastery."
        else:
            feedback = f"Valuable diagnostic data obtained. While {st.session_state.student_name} shows potential, the results suggest a need for reinforcing foundational principles. We recommend bridging these conceptual gaps before attempting higher-level competitive drills."
        st.write(feedback)
        st.info(f"**Path Forward:** Focus on '{df.sort_values('Score').iloc[0]['Category']}' for optimal growth.")
    ...
    if sheet:
        try:
            ...
            st.success("Analysis archived in cloud database.")
        except Exception:
            st.warning("Database connection failed; scores shown but not archived.")

    with st.expander("Comprehensive Solution Guide"):
        for idx, q in enumerate(st.session_state.selected_quiz):
